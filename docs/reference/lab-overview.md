...
- **`gitea/vertex-bank-app`**: mirrored from GitHub once at deploy time. Fleet polls this repo every 15 seconds to check for workload changes. No GitHub access is needed after deploy.
...
Creates a `GitRepo` resource in `fleet-default` namespace pointing at `http://192.168.122.20:3000/gitea/vertex-bank-app.git`. The target selector matches any cluster with labels `demo=true` and `edge-type=x86-cluster`. When students label a cluster in Exercises 5 and 6, Fleet picks it up automatically.
